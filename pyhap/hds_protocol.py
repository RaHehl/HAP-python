...
def _encode_length_prefixed(value: bytes, base: int, len8, len16, len32, len64) -> bytes:
    length = len(value)
    if length <= 32:
...
def encode(value: Any) -> bytes:
    ...
    if isinstance(value, str):
        return _encode_length_prefixed(
            value.encode("utf-8"), _UTF8_0, _UTF8_LEN8, _UTF8_LEN16,
            _UTF8_LEN32, _UTF8_LEN64,
        )
...
class _Decoder:
    ...
    def decode(self) -> Any:
        ...
        if _ARRAY_0 <= tag <= _ARRAY_14:
            return [self.decode() for _ in range(tag - _ARRAY_0)]
        ...
        if _DICT_0 <= tag <= _DICT_14:
            return {self.decode(): self.decode() for _ in range(tag - _DICT_0)}
